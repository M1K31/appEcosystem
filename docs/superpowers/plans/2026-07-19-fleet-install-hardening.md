...
| 4 | **P1** | cyber-harness (8088) has no install/uninstall; registry advertises a dead endpoint |
...
### Task 4: Bring the cyber-harness daemon (8088) into the install convention

`CybersecurityTeam/cyber-claude-agents` is a real ecosystem member — `ecosystem.yaml` statically registers `aegissiem_daemon` at `localhost:8088` as the "preferred security analysis backend" — but the repo has no install or uninstall script, nothing runs on 8088, and the registry hands consumers a dead endpoint.
...
**Interfaces:**
- Consumes: shared secret at `~/.config/ecosystem/secret.env`; `appEcosystem/auth/python` + `packages/ecosystem-client` for inter-service auth.
- Produces: launchd label `com.smartindustries.cyber-harness`, runtime at `~/.local/share/cyber-harness/venv`, service on port 8088 with health at `/api/status`.
...
```
Expected now: `000`/`ERR` (nothing listening).
...
#
# Runtime lives on the INTERNAL disk: the repo may sit on an external volume and
# a force-unmount there crashes mmap'd C-extensions with SIGBUS.
#
#   ./scripts/install-local.sh            # venv + deps
...
"$VENV/bin/pip" install -r "$REPO/daemon/requirements.txt"
"$VENV/bin/pip" install "$REPO"

# Shared ecosystem packages (inter-service auth + discovery).
...
if [ -d "$ECO_ROOT" ]; then
    ...
    echo "!!  appEcosystem not found at $ECO_ROOT — inter-service auth unavailable" >&2
fi
...
echo "==> Done. Interpreter: $VENV/bin/python"
```

- [ ] **Step 3: Create the full-purge uninstaller**

Create `CybersecurityTeam/cyber-claude-agents/scripts/uninstall.sh`:

```bash
#!/usr/bin/env bash
# Remove the cyber-harness daemon completely (FULL removal by default):
# service, internal-disk runtime, logs, and local state/DBs.
#
#   ./scripts/uninstall.sh              # FULL removal (asks to confirm)
#   ./scripts/uninstall.sh --yes        # no prompt (CI/automation)
#   ./scripts/uninstall.sh --dry-run    # print what would happen
#   ./scripts/uninstall-keep-data.sh    # keep state/DBs
set -euo pipefail
...
if $KEEP_DATA; then
    echo "==> Uninstalling cyber-harness — keeping state in $STATE_DIR."
else
    echo "==> Uninstalling cyber-harness — FULL removal (deletes local state/DBs)."
    if [ -z "$NONINTERACTIVE" ] && ! $DRY; then
        read -rp "Type 'yes' to permanently delete cyber-harness data: " confirm
...
run "rm -rf \"$PREFIX\""
run "rm -rf \"$HOME/Library/Logs/CyberHarness\""
...
echo "==> cyber-harness uninstalled. Reinstall with: ./scripts/install-local.sh --plist"
```

- [ ] **Step 4: Create the keep-data variant**
...
- [ ] **Step 5: Make executable and syntax-check**

```bash
cd /Volumes/Locker2/GitHub/CybersecurityTeam/cyber-claude-agents
chmod +x scripts/install-local.sh scripts/uninstall.sh scripts/uninstall-keep-data.sh
...
```
Expected: three `OK` lines, then a dry-run listing that touches only `~/.local/share/cyber-harness`, `~/.cyber-harness`, the plist, and the log dir.

- [ ] **Step 6: Install and verify 8088 answers**
...
If the daemon needs config before it will bind, read `daemon/aegissiem_daemon.py` for its entry point and required settings, and add the minimal config creation to `install-local.sh` (mirroring how `LogAnalysis/scripts/install-local.sh` seeds `~/.aegissiem/config.yml` from `config.example.yml`).

- [ ] **Step 7: Verify the registry now reports it healthy**
...
"
```
Expected: `aegissiem_daemon http://localhost:8088 healthy= True`

- [ ] **Step 8: Commit**

```bash
cd /Volumes/Locker2/GitHub/CybersecurityTeam/cyber-claude-agents
git add scripts/install-local.sh scripts/uninstall.sh scripts/uninstall-keep-data.sh
git commit -m "feat(install): add install/uninstall scripts for the harness daemon

ecosystem.yaml registers aegissiem_daemon at :8088 as the preferred security
backend, but the repo had no install path — nothing ran there and the registry
advertised a dead endpoint. Adds an internal-disk installer with a LaunchAgent
plus the standard full-purge and keep-data uninstallers."
```
