...
- [~] **Phase E — Hardening parity / client re-sync** (in progress):
  ...
  - [ ] **OpenEye**: branch `claude/mystifying-gauss-3aefaf` confirmed **fully superseded by main** (WebSocket auth, the 898-line test suite, and Py3.9 compat are all already on main via PR #7) → **safe to drop, nothing to salvage**. Remaining: stash old-scheme WIP, convert vendored→shared v0.3.0, drop the stale branch + embedded worktree.
  - [ ] **MagicMirror (JS)**: WIP reviewed — it's **old-scheme** (`signPayload` + a `_post`/`_delete` headers refactor), superseded by the v0.3.0 `ecosystem_client_js`. Convert: replace `js/ecosystem-auth`/`ecosystem-client` with the v0.3.0 JS; stash WIP for review.
