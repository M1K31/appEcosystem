...
- [~] **Phase E — Hardening parity / client re-sync** (in progress):
  ...
  - [ ] **MagicMirror (JS)**: WIP reviewed — old-scheme (`signPayload` + `_post`/`_delete` headers refactor), superseded by the v0.3.0 `ecosystem_client_js`. **Awaiting go-ahead** to convert: replace `js/ecosystem-auth`/`ecosystem-client` with the v0.3.0 JS (stash WIP for review).
